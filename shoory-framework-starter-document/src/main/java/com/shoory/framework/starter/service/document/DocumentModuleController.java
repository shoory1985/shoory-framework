...
public class DocumentModuleController {
	...
	@GetMapping(value = "/doc/module", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public String methodInfo(HttpServletRequest request, HttpServletResponse response) {
		documentUtils.ready();
		return pojoUtils.toJson(documentUtils.getMapModule());
	}
	
	@GetMapping(value = "/doc/module/{module}", produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	public String methodInfo(@PathVariable("module") String module, HttpServletRequest request, HttpServletResponse response) {
		documentUtils.ready();
		return pojoUtils.toJson(documentUtils.getMapModule().get(module));
	}
